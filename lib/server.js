...
var parseRegexp = new RegExp('\/api\/database\/([a-zA-Z0-9_-]+)(\/collection\/([a-zA-Z0-9_-]+)(\/(.*))?)?');
...
var parseData = function(data) {
  ...
  for (var i in data) {
    if (data.hasOwnProperty(i) && ['apikey', 'user', 'sort', 'order', 'limit', 'offset'].indexOf(i) === -1) {
      cleanData[i] = data[i];
    }
  }
...
var proccessRequest = function(data, infos, request, response) {
  ...
  req.toArray(function(err, items) {
    ...
    switch (request.method) {
      ...
      case "PUT":
        collection.findAndModify(query, {}, {$set:data}, {new: true}, function(err, updateData){
          if (err) {
            console.log(err);
